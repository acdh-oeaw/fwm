    ...
    def get_images(self):
        ...
        for x in images:
            if x.image_stream is not None:
                ...
                Images.pictures(x.easydb_id, x.id)
...
class Images(models.Model):
    ...
    def pictures(easy_db, image_id):
        ...
        # Update/Cache image path in DB
        Images.objects.filter(id=image_id).update(
            image_stream=file_path, description=des
        )

        # return dictionary with data
        help_im["beschreibung"] = des
        help_im["stream"] = file_path
        return help_im
    ...
    def get_image_file_name(self):
        if self.image_stream:
            _, tail = os.path.split(self.image_stream)
            return tail
        else:
            return None
