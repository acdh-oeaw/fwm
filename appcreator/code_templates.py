...
class {{ x.model_name }}ListFilter(django_filters.FilterSet):
    ...
    class Meta:
        ...
        fields = [
            'id',
            'legacy_id',{% for y in x.model_fields %}
            '{{ y.field_name }}',{% endfor %}
        ]
...
from . forms import (
{%- for x in data %}
    {{ x.model_name }}Form, {{ x.model_name }}FilterFormHelper{{ "," if not loop.last }}
{%- endfor %}
)
from . tables import (
{%- for x in data %}
    {{ x.model_name }}Table{{ "," if not loop.last }}
{%- endfor %}
)
from . models import (
{%- for x in data %}
...
{% else %}
    ...
    def get_source_table(self):
        return None
{% endif %}{% if x.natural_primary_key %}
    @classmethod
